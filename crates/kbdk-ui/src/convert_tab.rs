...
pub fn show(app: &mut KbdkApp, ui: &mut egui::Ui) {
    ...
    let mut model = app.saved_model.clone().unwrap_or_else(|| {
        app.workers.repo_root.join(&app.f.model_out).display().to_string()
    });

    egui::Grid::new("conv_grid").num_columns(3).spacing([10.0, 8.0]).show(ui, |ui| {
        ui.label("TorchScript model");
        ...
        ui.label("Dataset (labels + calibration)");
